...
if [ -f "./.claude/get-shit-done/VERSION" ]; then
  ...
  echo "LOCAL"
elif [ -f ~/.codex/skills/gsd-codex/get-shit-done/VERSION ]; then
  cat ~/.codex/skills/gsd-codex/get-shit-done/VERSION
  echo "GLOBAL"
else
  echo "UNKNOWN"
fi
...
(Paths are relative to your install location: `~/.codex/skills/gsd-codex/` for global, `./.claude/` for local)
...
**If GLOBAL install:**
```bash
rm -f ~/.codex/skills/gsd-codex/cache/gsd-update-check.json
```
</step>
